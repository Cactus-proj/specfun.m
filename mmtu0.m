...
%     ============================================================
kf=[];m=[];q=[];x=[];csf=[];csd=[];
fprintf(1,'%s \n','please enter kf, m, q and x(in degrees)');
%     READ(*,*)KF,M,Q,X
kf=1;
m=5;
q=25.0;
x=40;
fprintf(1,[repmat(' ',1,1),'kf =','%2g',',  ','m =','%2g',',  ','q =','%5.1g',',  ','x =','%5.1g' ' \n'],kf,m,q,x);
fprintf(1,'%0.15g \n');
if(kf == 1)fprintf(1,'%s \n',' x(degs.)cem(x,q)cem''(x,q)'); end;
if(kf == 2)fprintf(1,'%s \n',' x(degs.)sem(x,q)sem''(x,q)'); end;
fprintf(1,'%s \n',' --------------------------------------');
[kf,m,q,x,csf,csd]=mtu0(kf,m,q,x,csf,csd);
fprintf(1,[repmat(' ',1,2),'%5.1g',repmat('%16.9g',1,2) ' \n'],x,csf,csd);
%format(1x,,i2,',  ',,i2,',  ',,f5.1,',  ',,f5.1);
%format(2x,f5.1,2f16.9);
end
function [kf,m,q,x,csf,csd]=mtu0(kf,m,q,x,csf,csd,varargin);
%     ===============================================================
...
km=fix(qm+0.5.*fix(m));
[kd,m,q,a,fg]=fcoef(kd,fix(m),q,a,fg);
ic=fix(fix(m)./2)+1;
...
for  k=1:km;
    ...
    if(k >= ic&abs(fg(k))< abs(csf).*eps)break; end;
end;
csd=0.0d0;
for  k=1:km;
    if(kd == 1);
    ...
    if(k >= ic&abs(fg(k))< abs(csd).*eps)break; end;
end;
return;
end
