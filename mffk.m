...
%     F±(x)and K±(x)using subroutine FFK
...
%     GA  --- Arg[K±(x)](Degs.)
%     Example:
%     x     Re[F±(x)]±Im[F±(x)]Mod[F±(x)]±Arg[F±(x)]
%     ----------------------------------------------------------
...
%     x     Re[K±(x)]±Im[K±(x)]Mod[K±(x)]±Arg[K±(x)]
%     ----------------------------------------------------------
...
x=10.0;
fprintf(1,'%0.15g \n');
fprintf(1,'%s ','   x      re[f±(x)]±im[f±(x)]');fprintf(1,'%s \n', 'mod[f±(x)]±arg[f±(x)]');
...
fprintf(1,'%0.15g \n');
fprintf(1,'%s ','   x      re[k±(x)]±im[k±(x)]');fprintf(1,'%s \n', 'mod[k±(x)]±arg[k±(x)]');
fprintf(1,'%s ',' ---------------------------------------');fprintf(1,'%s \n','-----------------------');
fprintf(1,[repmat(' ',1,1),'%5.1g',repmat('%14.8g',1,3),'%14.6g' ' \n'],x,gr,gi,gm,ga);
